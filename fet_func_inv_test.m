...
for i=1:vinlen
    ...
    vin=Vinlis(i);
    fsfun=@(vo) fsfun0(vin,vo,Vdd,lablis);
    vo=fsolve(fsfun,x0);
    voumat(i,:)=vo;

    idm=fsfun1(vin,vo,Vdd,lablis);
    iddmat(i,:)=idm;
end
...
% legend({'port-2','port-3'},'FontSize',12)
% 
% figure
...
function res=fsfun0(vin,vo,Vdd,lablis)

idm=fsfun1(vin,vo,Vdd,lablis);% row vo expected
...
function idm=fsfun1(vin,vo,Vdd,lablis)
...
hd_str=sprintf('fet_func%d%d',lab);
hd=str2func(hd_str);

ids=feval(hd,Vg-Vs,Vd-Vs);
